...
if (!process.env.TOKEN) {
	console.error(chalk.red('Missing TOKEN in .env. Please set TOKEN=<your discord authorization token>'));
	process.exit(1);
}

const rawToken = process.env.TOKEN.trim().replace(/^Bot\s+/i, '').replace(/^Bearer\s+/i, '');
if (!rawToken) {
	console.error(chalk.red('Invalid TOKEN in .env. Please set TOKEN to a valid Discord authorization token.'));
	process.exit(1);
}

const client = new ClientQuest(rawToken);
...
async function start() {
	const user = (await client.rest.get('/users/@me')) as UserInfo;
	...
	results.forEach((result, index) => {
		...
		if (result.status === 'fulfilled') {
			const fulfilledResult = result as PromiseFulfilledResult<unknown>;
			const rewardValue =
...
start().catch((error) => {
	if ((error as any)?.status === 401) {
		console.error(chalk.red('Startup failed: Unauthorized.'));
		console.error(chalk.red('Please verify that your .env TOKEN is a valid Discord user authorization token.'));
		console.error(chalk.red('Do not use a bot token unless this project is modified for bot authentication.'));
	} else {
